...
d3.tsv("./typhon.tsv", function(err, typhonData){
  ...
  return d3.tsv("./data/" + name + ".tsv", function(err, tsvData){
    ...
    scaleX = d3.time.scale().range([0, sum_barwidth]).domain(d3.extent(tsvData.map(function(it){
      return it.date;
    })));
    scaleQ = d3.scale.linear().domain(domainQ).range([0, sum_barwidth]);
    ...
    return $(document).ready(function(){
      ...
      return d3.selectAll(".oversvg").style({
        "position": "fixed",
        "top": cnvs.top,
        "left": cnvs.left
      }).call(brush);
