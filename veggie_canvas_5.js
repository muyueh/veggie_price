...
var ref$, flatten, sum, drop, each, take, map, keys, listsToObj, maxMlt, parseDate, shNaN, typShNaN, ggl, c, color, view, lsmkt, lssbtype, lstyphon, lscolor, lsveggie, lsgdname, tbgdname, enname, lsenname, datechage, datebetween;
...
tbgdname = listsToObj(lsveggie, lsgdname);
...
d3.tsv("./typhon.tsv", function(err, typhonData){
  ...
  return d3.tsv("./wo_out_data/" + name + ".tsv", function(err, tsvData){
    ...
    updateGraph = function(){
      ...
      clear();
      clear_sm();
      d3.selectAll(".count").text(yearDim.top(Infinity).length);
      chrtPMax = getMax(lsPChrt) * maxMlt;
      chrtQMax = getMax(lsQChrt) * maxMlt;
    ...
    setList = function(holderName, listData, onClick){
      ...
      ncut = 12;
      cut = ~~(lg / ncut);
      return rw.selectAll("ul").data((function(){
        var i$, to$, results$ = [];
        for (i$ = 0, to$ = ncut - 1; i$ <= to$; ++i$) {
          results$.push(i$);
        }
        return results$;
      }())).enter().append("ul").attr({
        "class": "list-unstyled col-lg-" + ~~(12 / ncut)
      }).each(function(c){
        return d3.select(this).selectAll("li").data(listData.filter(function(it, i){
          return i > c * cut && i < (c + 1) * cut;
