...
chrt-p-max = get-max ls-p-chrt
chrt-q-max = get-max ls-q-chrt
...
own-color = -> 
	it.selectAll "rect.bar" .each -> 
		d3.select this 
			.attr {
				# "style": "fill:" + ggl.bck + "; stroke:" + color it.x
				"style": "fill:" + color it.x
			}
...
bpy = barPriYear.width barWidth
	...
	.y d3.scale.linear!.domain [0, domain-p[1] * 0.4]
...
bpm = barPriMonth.width barWidth + 120
	.height barHeight
	.margins marginJson
	.renderlet ->
		render-act it
...
bpm
	.xAxis!.tickFormat -> if (it + 1) <= 12 then (it + 1) else ""
...
bqy = barQuanYear.width barWidth
	...
	.group quanYear
	.valueAccessor -> it.value.average
	.y d3.scale.linear!.domain [0, domain-q[1] * 0.4]
	.x(d3.scale.linear!.domain(year-ext))	
	.colors -> colorBike2
	.on("filtered", -> updateGraph!)
...
bqm = barQuanMonth.width barWidth + 120
	.height barHeight
	.margins marginJson
	.renderlet ->
		render-act it
