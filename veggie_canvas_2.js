...
var ref$, drop, each, take, map, keys, listsToObj, parseDate, shNaN, typShNaN, ggl, c, color, view, lsmkt, lssbtype, lstyphon, lscolor, lsveggie, enname, lsenname, datechage, datebetween;
...
d3.tsv("./typhon.tsv", function(err, typhonData){
  ...
  return d3.tsv("./data/" + name + ".tsv", function(err, tsvData){
    var margin, w, h, domainQ, domainP, scaleX, scaleQ, scaleY, line, fltrData, canvas, canvasWidth, canvasHeight, ctx, canvasData, buf8, drawPixel, clear, updateCanvas, weekDayTable, barPriYear, barPriMonth, barPriMkt, barPriSb, barPriTyp, barQuanYear, barQuanMonth, barQuanMkt, barQyanSb, barQuanTyp, ndx, all, sml, yearDim, monthDim, weekdayDim, mktDim, sbDim, tyDim, averageAdd, averageRemove, averageInitial, averageQuanAdd, averageQuanRemove, averageQuanInitial, priceYear, priceMonth, priceMkt, priceSb, priceTy, quanYear, quanMonth, quanMkt, quanSb, quanTy, lsPChrt, lsQChrt, getMax, chrtPMax, chrtQMax, barWidth, barHeight, marginJson, colorBike, colorBike2, yearExt, renderAct, emptyC, removeZero, ownColor, bpy, bpm, bqy, bqm, updateGraph, setList;
    ...
    scaleY = d3.scale.linear().domain([0, domainP[1]]).range([h, 0]);
    ...
    chrtPMax = getMax(lsPChrt);
    chrtQMax = getMax(lsQChrt);
    barWidth = 150;
    barHeight = 120;
    marginJson = {
      "top": 10,
    ...
    }).y(d3.scale.linear().domain([0, domainP[1] * 0.4])).x(d3.scale.linear().domain(yearExt)).colors(function(){
      return colorBike;
    ...
    bpy.yAxis().ticks(3);
    bpy.xAxis().ticks(3);
    bpm = barPriMonth.width(barWidth + 120).height(barHeight).margins(marginJson).renderlet(function(it){
      renderAct(it);
      emptyC(it, colorBike);
      return removeZero(it);
    }).dimension(monthDim).group(priceMonth).valueAccessor(function(it){
      return it.value.average;
    }).y(d3.scale.linear().domain([0, domainP[1] * 0.4])).x(d3.scale.linear().domain([-1, 12])).centerBar(true).colors(function(){
      return colorBike;
    }).on("filtered", function(){
      return updateGraph();
    });
    ...
    barPriMkt.width(keys(lsmkt).length * 25 + marginJson.left + marginJson.right).height(barHeight).margins(marginJson).renderlet(function(it){
      ...
      return it.value.average;
    }).y(d3.scale.linear().domain([0, domainP[1] * 0.4])).x(d3.scale.ordinal().domain(keys(lsmkt))).xUnits(dc.units.ordinal).gap(10).colors(colorBike).on("filtered", function(){
      return updateGraph();
    }).yAxis().ticks(3);
    barPriSb.width(keys(lssbtype).length * 25 + marginJson.left + marginJson.right).height(barHeight).margins(marginJson).renderlet(function(it){
      ...
      return it.value.average;
    }).y(d3.scale.linear().domain([0, domainP[1] * 0.4])).x(d3.scale.ordinal().domain(keys(lssbtype))).xUnits(dc.units.ordinal).gap(15).colors(colorBike).on("filtered", function(){
      return updateGraph();
    }).yAxis().ticks(3);
    barPriTyp.width(barWidth).height(barHeight).margins(marginJson).renderlet(function(it){
      ...
      return it.value.average;
    }).y(d3.scale.linear().domain([0, domainP[1] * 0.4])).x(d3.scale.ordinal().domain(["na", "-10", "-5", "T", "+5"])).xUnits(dc.units.ordinal).gap(10).colors(colorBike).on("filtered", function(){
      return updateGraph();
    }).yAxis().ticks(3);
    bqy = barQuanYear.width(barWidth).height(barHeight).margins(marginJson).renderlet(function(it){
      ...
      return it.value.average;
    }).y(d3.scale.linear().domain([0, domainQ[1] * 0.4])).x(d3.scale.linear().domain(yearExt)).colors(function(){
      return colorBike2;
    }).on("filtered", function(){
      return updateGraph();
    });
    bqy.yAxis().ticks(3);
    bqy.xAxis().ticks(3);
    bqm = barQuanMonth.width(barWidth + 120).height(barHeight).margins(marginJson).renderlet(function(it){
      renderAct(it);
      emptyC(it, colorBike);
      return removeZero(it);
    }).dimension(monthDim).valueAccessor(function(it){
      return it.value.average;
    }).group(quanMonth).y(d3.scale.linear().domain([0, domainQ[1] * 0.4])).x(d3.scale.linear().domain([-1, 12])).centerBar(true).colors(function(){
      return colorBike2;
    }).on("filtered", function(){
      return updateGraph();
    });
    ...
    barQuanMkt.width(keys(lsmkt).length * 25 + marginJson.left + marginJson.right).height(barHeight).margins(marginJson).renderlet(function(it){
      ...
      return it.value.average;
    }).group(quanMkt).y(d3.scale.linear().domain([0, domainQ[1] * 0.4])).x(d3.scale.ordinal().domain(keys(lsmkt))).xUnits(dc.units.ordinal).gap(10).colors(colorBike2).on("filtered", function(){
      return updateGraph();
    }).yAxis().ticks(3);
    barQyanSb.width(keys(lssbtype).length * 25 + marginJson.left + marginJson.right).height(barHeight).margins(marginJson).renderlet(function(it){
      ...
      return it.value.average;
    }).group(quanSb).y(d3.scale.linear().domain([0, domainQ[1] * 0.4])).x(d3.scale.ordinal().domain(keys(lssbtype))).xUnits(dc.units.ordinal).gap(15).colors(colorBike2).on("filtered", function(){
      return updateGraph();
    }).yAxis().ticks(3);
    barQuanTyp.width(barWidth).height(barHeight).margins(marginJson).renderlet(function(it){
      ...
      return it.value.average;
    }).y(d3.scale.linear().domain([0, domainQ[1] * 0.4])).x(d3.scale.ordinal().domain(["na", "-10", "-5", "T", "+5"])).xUnits(dc.units.ordinal).gap(10).colors(colorBike2).on("filtered", function(){
      return updateGraph();
    }).yAxis().ticks(3);
    ...
    return setList(".index-holder", lsveggie, function(it){
      return window.open('./index_op1.html#' + it, '_blank');
    });
  });
});